...
function isUnit(txt){
  ...
  while(index < txt.length){
    ...
    for(var i=0;i<unit.length;i++){
      if(index + unit[i].length > txt.length) continue;
      if(txt.substring(index, index + unit[i].length) == unit[i]){
        index += unit[i].length;
        flag = false;
        break;
      }
...
function WordLabeling(txt){
  ...
  var exception = ["In", "At", "As"];
  for(var i=0;i<exception.length;i++){
    if(txt == exception[i]){
      return {"tag":"o", "level":2, "similar":exception[i]};
    }
  }
