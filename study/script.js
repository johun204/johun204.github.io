...
function init2(){
	...
	if(document.getElementsByTagName("h2").length > 0){
		document.getElementsByTagName("h2")[0].innerHTML = document.getElementsByTagName("h2")[0].innerHTML + "<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label><input type=checkbox id='cb_hideDup' onchange='hideDup(this.checked)'><span id='lb_chk'>중복 문제 가리기</span></label>" + "<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label><input type=checkbox id='cb_shuffle' onchange='shuffle(this.checked)'><span id='lb_chk2'>문제 보기 섞기</span></label>";
		if(check_key(false)){
			hideDup(true);
			document.getElementById("cb_hideDup").checked = true;
		}
	}
	
	if(check_key(false)){
		shuffle(true);
		document.getElementById("cb_shuffle").checked = true;
	}
}
function check_key(popup){
	return true;
}
function f(n){
	if(!check_key(true)) return false;
...
function hideDup(t){
	//중복문제 가리기
	if(t == true && !check_key(true)){
		document.getElementById("cb_hideDup").checked = false;
		return false;
	}
	var cnt=0;
	if(t == true){
...
function shuffle(t){
	//문제 보기 섞기
	if(t == true && !check_key(true)){
		document.getElementById("cb_shuffle").checked = false;
		return false;
	}
	var cnt=0;
	if(t == true){
